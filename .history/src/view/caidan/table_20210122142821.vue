...
export default {
  ...
  methods: {
    filterTagTable(filters) {
      ...
      this.tableData = [];
      this.bb = [];
      let windows = filters.windows;
      windows.forEach((item) => {
        this.shaixuan(item); // 筛选所选项下的所有数据，需要调用后端接口
      });
    },
    shaixuan: function (windows) {
      var that = this;
      this.$get({
        url: this.$ApiList.ShaiXuanCaidanList,
        data: {
          windows: windows,
        },
      })
        .then((res) => {
          ...
          if (res.rtnCode === 0) {
            ...
            that.Sum = this.bb.length;
            console.log("Sum", that.Sum);
            setTimeout(() => {
              this.loading = false;
    ...
    CloseDialog: function () {
      ...
      if (this.typetext === "add") {
        ...
        this.$post({
          url: this.$ApiList.UpdataCaidanList,
          data: this.selectdata,
        })
          .then((res) => {
            console.log("res", res);
    ...
    hanldeAdd(type, list) {
      ...
      if (type === "add") {
        (this.dialogtitle = "添加"), (this.selectdata = {});
        this.typetext = "add";
      } else {
        (this.dialogtitle = "编辑"), (this.selectdata = list);
        this.typetext = "editor";
      }
