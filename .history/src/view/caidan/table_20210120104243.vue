...
export default {
  ...
  methods: {
    ...
    handleCurrentChange(val) {
      ...
      this.pageSize = val;
      this.windowsList = [];
      this.fenyeList[this.pageSize-1].forEach((item) => {
        this.windowsList.push(item);
      });
      console.log("测试数据", this.pageSize-1, this.windowsList,this.fenyeList)
    },
      ...
      this.$get({
        ...
        .then((res) => {
          if (res.rtnCode === 0) {
            ...
            that.fenyeList = this.fenye(res.data.table, 10);
            that.fenyeList[this.pageNo].forEach((item) => {
