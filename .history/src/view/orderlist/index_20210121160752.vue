  ...
  <div>
    ...
    >
      ...
      <el-table-column label="查看评价" width="100"
        ><el-button @click="LookPinglun(scope.row.orderid)">
          详情
...
export default {
  ...
  methods: {
    // 查看评价
    LookPinglun: function (id) {
      this.SumData.forEach((item) => {
        if (item.orderid === id) {
          this.pingjia = item.pingjia;
          this.pingfen = item.pingfen;
          this.pingjiaimage = JSON.parse(item.pingjiaimage);
          this.pingjiadate = this.$gettime(item.pingjiadate);
