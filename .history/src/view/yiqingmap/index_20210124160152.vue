  ...
  methods: {
    loaddata: function () {
      ...
      this.$get({
        ...
        .then((res) => {
          if (res.codeid === 10000) {
            that.mydata = res.retdata;
          console.log("that.mydata", that.mydata);
            that.mydata.forEach(item=>{
              if(item.subList.length>0){
                item.subList.forEach(iitem=>{
                  console.log('that.getAddressCode(iitem.city)',that.getAddressCode(iitem.city))
                  let codelist = that.getAddressCode(iitem.city);
                  iitem.lng = codelist.lng;
                  iitem.lat = codelist.lat;
                })
              }
            })
            console.log("that.mydata",that.mydata);
    ...
    getAddressCode: function (address) {
      this.$axios({
        url: "http://api.map.baidu.com/geocoder?address="+address+"&output=json&key=ccrbBUEq6Ai1FY3KxhdbFCjXdWZu2RF8",
        method: "get",
        params: {},
      }).then((res) => {
        console.log(res);
