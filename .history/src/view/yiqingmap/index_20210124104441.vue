<template>
    <div id="mapChart" class="mapChart" style="width: 100%;height: 100%;top: 2%;left:0%"></div>
</template>
...
name: 'mapChina',
  ...
  methods: {
      mapFn(){
        ...
        var mapBoxEchart = this.$echarts.init(document.getElementById('mapChart'));
        var data = this.cityData;
        var seriesData = [];
        var geoCoordMap = this.geoCoordMap;
            
            var convertData = function (data) {//散点值和市区经纬度组合
                var res = [];
  ...
  created() {
    ...
    this.sortCityData = this.cityData.sort(function (a, b) {
                            return b.value - a.value;
                       }).slice(0, 6)
  },
  components: {
