<template>
  <div>
    ...
    <!-- APP -->
      ...
      <el-form-item label="版本号2" prop="versionnum">
        <el-input
...
export default {
  ...
  methods: {
    ...
    getFile(event) {
      ...
      this.$ajax({
        method: "POST",
        url: "http://42.193.106.104:8888/MultiFileUpload",
        data: formData,
      }).then((res) => {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$ajax({
            method: "GET",
            url: 
            "http://42.193.106.104:8888/api/UnloadApp/MultiFileAppUploadd?Appid="+this.form.appid+"&Fileurl="+this.form.fileurl
            +"&Note="+this.form.note+"&Version="+this.form.version+"&Versionnum="+this.form.versionnum,
          }).then((res) => {
            console.log("res", res);
          });
        } else {
