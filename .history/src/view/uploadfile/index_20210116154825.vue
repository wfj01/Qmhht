<template>
  ...
  <form>
    ...
    <input type="text" value="" v-model="note" placeholder="请输入note" />
...
export default {
  ...
  methods: {
    ...
    submit(event) {
      ...
      this.$ajax({
        method: "post",
        url: "http://localhost:33527/MultiFileAppUpload?appid="+this.appid+"&version="+this.version+"&versionnumber="+this.versionnumber+"&note="+this.note,
        data: formData,
      }).then((resp) => {
