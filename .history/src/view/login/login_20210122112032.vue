    ...
    handleSubmit(event) {
      ...
      this.$refs.ruleForm2.validate((valid) => {
        if (valid) {
          ...
          if (this.ruleForm2.imageCode !== imagetext) {
            this.$message.error("验证码输入不正确!");
            return;
          } else {
            console.log("this.checked", this.checked);
            this.$get({
              url: this.$ApiList.getLoginPage,
              data: {
                Name: this.ruleForm2.username,
                Password: this.ruleForm2.password,
                License: this.ruleForm2.License,
              },
            })
              .then((res) => {
                console.log("res", res);
                if (res.rtnCode === 0) {
                  this.$message.success("登录成功,正跳转至主页面!");
                  this.$router.push({
                    name: "TablePage",
                  });
                  if (this.checked === true) {
                    let userinfo = {};
                    userinfo.username = this.ruleForm2.username;
                    userinfo.password = this.ruleForm2.password;
